...
var apiLocation = "https://api.ssllabs.com/api/v4"
...
func invokeGetRepeatedly(url string) (*http.Response, []byte, error) {
	...
	for {
		...
		req.Header.Add("email", globalEmailAddress)
...
func invokeApi(command string) (*http.Response, []byte, error) {
	...
	for {
		...
		if resp.StatusCode == 400 {
			// 400 status code can be an invalid email address error check the error field to confirm
			apiError, err := getLabsErrorResponse(body)
			if err != nil {
				return nil, nil, err
			}
			apiResponseError := apiError.ResponseErrors[0]
			if apiResponseError.Field == "email" {
				log.Fatalf("[ERROR] %s", apiResponseError.Message)
			}
		}
...
func getLabsErrorResponse(body []byte) (*LabsErrorResponse, error) {
...
func main() {
	...
	var conf_email = flag.String("email", "", "Registered organization email address on SSLLabs")
